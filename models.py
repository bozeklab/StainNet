...
class StainNet(pl.LightningModule):
    def __init__(self, opt):
        ...
        n_layer = int(opt.n_layer)
        assert n_layer > 0
        ins = [opt.input_nc_net] + [opt.channels] * n_layer
        outs = [opt.channels] * n_layer + [opt.output_nc_net]
        for c_in, c_out in zip(ins, outs):
            model_list.append(nn.Conv2d(c_in, c_out, kernel_size=KERNEL_SIZE, bias=True, padding="same"))
            model_list.append(nn.ReLU(True))

        model_list = model_list[:-1] # remove last ReLU activation
        self.pipeline = nn.Sequential(*model_list)
        self.loss_inner = nn.L1Loss()
        self.loss_fun = lambda y, y_hat: torch.log(1 + self.loss_inner(y, y_hat))
        self.stain_gan = None
